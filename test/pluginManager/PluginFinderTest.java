...
public class PluginFinderTest {
	...
	public void testRefreshingDuringLaunch() throws InterruptedException {
		...
		assertTrue(tmp > 0);
		PluginMover.move(new File("bin/plugins/ToLowerCase.class"), new File(
				"bin/plugins/testMove/ToLowerCase.class"));
		new File("bin/plugins/ToLowerCase.class").delete();
		Thread.sleep(3000);
		assertTrue(tmp != pluginFinder.files.length);
		PluginMover.move(new File("bin/plugins/testMove/ToLowerCase.class"),
				new File("bin/plugins/ToLowerCase.class"));
